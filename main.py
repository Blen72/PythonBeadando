...
class Tile():
    ...
    def draw(self):
        ...
        #window.blit(font.render(str(self.groupn), True, (0,0,0)), (self.x, self.y))
        #font = pg.font.SysFont("arial", 8)
        #window.blit(font.render(str(self.y//TILE_SIZE)+","+str(self.x//TILE_SIZE), True, (128,255,0)), (self.x, self.y))
...
class Entity():
    ...
    def move(self, x, y):
        ...
        #print(self.y//TILE_SIZE+y,MH,currentScreen.map[self.y//TILE_SIZE+y])
        #print(self.x//TILE_SIZE+x,MW,currentScreen.map[self.y//TILE_SIZE+y][self.x//TILE_SIZE+x])
        if 0<=self.y//TILE_SIZE+y<MH and 0<=self.x//TILE_SIZE+x<MW and (currentScreen.map[self.y//TILE_SIZE+y][self.x//TILE_SIZE+x].type==0 or currentScreen.map[self.y//TILE_SIZE+y][self.x//TILE_SIZE+x].type==2):
            self.x+=x*TILE_SIZE
            self.y+=y*TILE_SIZE
...
class ScreenMainMenu(Screen):
    ...
    def draw(self):
        ...
        #Buttons
        color = (0, 192, 0)
        self.buttons["difficulty"]=pg.draw.rect(window, color, pg.Rect(16, 140, 60+get_val_by_difficulty(35,55,33), 24))
        ...
        window.blit(font.render("Difficulty: "+get_val_by_difficulty("easy", "medium", "hard"), True, (0,0,0)), (16,140))
...
class ScreenLevel(Screen):
    ...
    def generate_map(self):
        ...
        while len(groupset)!=1:
            ...
            if 0<=dx<MH and 0<=dy<MW and m[rx][ry].groupn!=m[dx][dy].groupn and m[wallx][wally].groupn==0:
                ...
                if min(m[rx][ry].groupn, m[dx][dy].groupn) == m[rx][ry].groupn:
                    min_tile = m[rx][ry]
                    #m[dx][dy].groupn=min_tile.groupn
                    #m[dx][dy].refresh_group(min_tile)
                    min_tile.get_adam().parent=m[dx][dy].get_genz()
                else:
                    min_tile = m[dx][dy]
                    #m[rx][ry].groupn=min_tile.groupn
                    #m[rx][ry].refresh_group(min_tile)
                    min_tile.get_adam().parent=m[rx][ry].get_genz()
                m[wallx][wally].change(min_tile, GRUOND_TILE_IMG, 0)
                min_tile.get_adam().parent = m[wallx][wally]#.get_genz()
                m[wallx][wally].refresh_group(min_tile)
                #print(m[rx][ry].groupn,m[wallx][wally].groupn,m[dx][dy].groupn)
        #print(groupset)
        ex,ey=randloc()
        ...
        #print(WHEIGHT/ TILE_SIZE)
        #m.append([Tile(WALL_TILE_IMG,j*TILE_SIZE,0,1) for j in range(WWIDTH // TILE_SIZE)])
        #for i in range(1,WHEIGHT // TILE_SIZE-1):
        #    m.append([Tile(WALL_TILE_IMG,0,i*TILE_SIZE,1)])
        #    for j in range(1,WWIDTH // TILE_SIZE-1):
        #        if random()>0.5:
        #            m[i].append(Tile(WALL_TILE_IMG,j*TILE_SIZE,i*TILE_SIZE,1))
        #        else:
        #            m[i].append(Tile(GRUOND_TILE_IMG, j * TILE_SIZE, i * TILE_SIZE, 0))
        #    m.append([Tile(WALL_TILE_IMG, (WWIDTH//TILE_SIZE-1)*TILE_SIZE, i * TILE_SIZE, 1)])
        #m.append([Tile(WALL_TILE_IMG,j*TILE_SIZE,(WHEIGHT//TILE_SIZE-1)*TILE_SIZE,1) for j in range(WWIDTH // TILE_SIZE)])
        #print(m)
        return m
...
class ScreenGG(Screen):
    ...
    def add_particle(self):
        self.particles.append([(random() * 0xff, random() * 0xff, random() * 0xff), random() * WWIDTH, random()*10, 1])  # col,x,y,h
...
gameoverScreen=None
#testScreen=ScreenLevel(1,336*2-16,336,3)
currentScreen=mainmenuScreen#testScreenaw
currentScreen.init()
...
while running:
    ...
    if isinstance(currentScreen, ScreenLevel):
        ...
        for i in range(grad):
            ...
            window.blit(r, (player.x - currentScreen.view_radius, player.y + currentScreen.view_radius+TILE_SIZE-(i+1)*gradW))
            ...
            window.blit(r, (player.x + currentScreen.view_radius + TILE_SIZE - i * gradW, player.y - currentScreen.view_radius))

        if spacedCounter<spacedCounterMax:
            spacedCounter+=1
            draw_opacity_rect((0,0,0,127),TILE_SIZE,4,player.x,player.y+TILE_SIZE)
            draw_opacity_rect((0,255,0,127),TILE_SIZE*spacedCounter/spacedCounterMax,4,player.x,player.y+TILE_SIZE)
        #Gui element
        font = pg.font.SysFont("arial", 16)
        ...
        if player.x==currentScreen.endloc.get("x") and player.y==currentScreen.endloc.get("y"):
            ...
            if endless or currentScreen.lvl<8:
                currentScreen=ScreenLevel(currentScreen.lvl+1, min(currentScreen.ww+32, pg.display.get_desktop_sizes()[0][0]), min(currentScreen.wh+32, pg.display.get_desktop_sizes()[0][1]), max(currentScreen.view_radius/TILE_SIZE-1,3))
            else:
                currentScreen=ScreenGG()
            currentScreen.init()
            #currentScreen=None #end level
    pg.display.update()
pg.quit()
